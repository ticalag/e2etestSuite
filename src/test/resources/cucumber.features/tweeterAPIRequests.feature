...
Feature: Test Tweeter API Requests
  ...
  Scenario: Favorite Tweet is added to the favorite tweet list by API Response
    Given that we add as favorite tweet the 2nd posted tweet
    Then we should have a favorite tweet added to the list

  @profileUpdate
  Scenario: Profile update using API request
    Given that we update the users language and time zone to "en" and "London" Time Zone
    When we retrieve the account settings
    Then the language should be "en" and the time-zone "London"
